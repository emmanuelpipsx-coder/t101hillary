...
## Step 6 — Report real joins back to Meta (Conversions API)

This makes your Meta ad account see actual channel joins as "Lead" events,
instead of just button clicks — fixing the exact "22 leads, 0 joins" problem.

1. Go to **Meta Events Manager** → select your Pixel (`2217682802419563`) →
   **Settings** → find the **Conversions API** section → click **Generate access token**.
2. Copy that token.
3. In Railway → your project → **Variables** tab, add:
   - `META_PIXEL_ID` → `2217682802419563`
   - `META_CAPI_TOKEN` → the token from step 2
4. Save — Railway will redeploy automatically.
5. Test it: click your invite link, get approved, then go to Events Manager →
   **Test Events** tab (you'll need to also set `META_TEST_EVENT_CODE` from that
   tab as a temporary Railway variable while testing, then remove it once confirmed
   working) — you should see a "Lead" event show up within a few seconds.

**Important limitation to know**: because Telegram is a separate app from your
landing page, this server event can't carry the exact ad-click ID Meta normally
uses for tight attribution. It still gives Meta a real, accurate signal of "this
many actual joins happened" instead of counting every click — which is what was
inflating your numbers before. Over time, as more real Lead events flow in, Meta's
delivery system uses this to find people more likely to actually join, not just click.
